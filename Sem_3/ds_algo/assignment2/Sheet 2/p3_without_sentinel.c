...
struct list list_pool[size];
void display(int l);
//--------------------FUNCTIONS---------------------------------------------------------------

//CHECKS IF LIST IS EMPTY
int empty(int l){                      //RETURNS TRUE IF EMPTY
	struct list li=list_pool[l-1];
	if(li.len==0){return 1;}
	return 0;
};

//RETURNS LENGTH OF THE LIST
int length(int l){
	return list_pool[l-1].len;
};

//READS ELEMENT AT POSITION POS
int read(int l,int pos){
	return list_pool[l-1].items[pos];
};

//WRITES ELEMENT AT POSITION POS
void write(int l,int pos,int obj){
	struct list li=list_pool[l-1];
	if(li.len<pos){printf("INVALID OPERATION. INDEX DOESNOT EXIST.\n");}
	else{
		li.items[pos]=obj;
		list_pool[l-1]=li;
...
void insert(int l,int pos,int obj){
	...
	if(li.len<pos){printf("INVALID OPERATION. INDEX DOESNOT EXIST.\n");}
		...
		for(int i=li.len-1;i>=pos;--i){
			li.items[i+1]=li.items[i];
		};
		li.items[pos]=obj;
		li.len++;
		list_pool[l-1]=li;
...
void search(int l,int obj){
	if(empty(l)){printf("INVALID OPERATION. LIST EMPTY\n");}
		...
		for(int i=0;i<li.len;++i){
			if(li.items[i]==obj){printf("%d FOUND AT %d\n",obj,i);return;}
		};
		printf("NOT FOUND\n");
	};
};
...
void ascend(int l){
	...
	for(int i=1;i<m;++i){
		t=li.items[i];
		k=i-1;
		while(t<li.items[k] && k>=0){li.items[k+1]=li.items[k];k=k-1;};
		li.items[k+1]=t;
	};
	list_pool[l-1]=li;
};
...
void reverse(int l){
	...
	struct list li1;
	li1.len=li.len;
	for(int i=li.len-1;i>=0;--i){
		li1.items[li.len-1-i]=li.items[i];
	};
	list_pool[l-1]=li1;
};
